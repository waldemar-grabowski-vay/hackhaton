...
import {
  ...
  HARNESS_ORDER,
  CONNECTOR_HARNESS,
  telestationConnectorLocations,
} from "@/telestationConnectorLocations";

const CONNECTOR_ZOOM = 4;
const DRAG_THRESHOLD = 4;

export interface TelestationFocusTarget {
  connectorId: string;
}

interface TelestationDiagramProps {
  focusTarget?: TelestationFocusTarget;
}

export function TelestationDiagram({ focusTarget }: TelestationDiagramProps) {
  const [activeHarness, setActiveHarness] = useState<HarnessKey>("board");
  ...
  const focusLocation = focusTarget
    ? telestationConnectorLocations[activeHarness]?.[focusTarget.connectorId]
    : undefined;
  ...
  // When focusTarget changes, switch harness tab if needed then zoom to connector
  useEffect(() => {
    if (!focusTarget) {
      setZoom(1);
      containerRef.current?.scrollTo({ top: 0, left: 0, behavior: "smooth" });
    ...
    if (targetHarness !== activeHarness) {
      ...
      // Tab switch resets zoom; useEffect below handles scroll after mount
      pendingConnectorScroll.current = true;
      setZoom(CONNECTOR_ZOOM);
      return;
    }

    const loc = telestationConnectorLocations[activeHarness]?.[focusTarget.connectorId];
    if (!loc) return;
  ...
  // Reset zoom/scroll when switching tabs manually
  function switchHarness(h: HarnessKey) {
    setActiveHarness(h);
    setZoom(1);
        ...
        <div style={{ position: "relative", width: `${Math.round(zoom * 100)}%` }}>
          ...
          {focusLocation && (
            <div style={{ position: "absolute", left: `${focusLocation.fx * 100}%`, top: `${focusLocation.fy * 100}%`, pointerEvents: "none" }}>
              <div style={{ position: "absolute", width: 40, height: 40, borderRadius: "50%", border: "2.5px solid #ef4444", animation: "ts-pulse 1.6s ease-out infinite" }} />
