...
class ConnectionCreds:
    ...
    passphrase: str | None
    password: str | None
    # When True, the main window persists key_filename + user to disk on
    # accept so they don't have to be re-selected next launch.
    remember: bool = True
...
class ConnectionDialog(QDialog):
    def __init__(self, parent: QWidget | None, creds: ConnectionCreds, error: str | None = None):
        ...
        self.remember_check = QCheckBox("Remember key file + user (saved to %USERPROFILE%\\.tsdiag\\settings.json)")
        self.remember_check.setChecked(creds.remember)
