...
# Matches "TS_ChA_ERRQ_Byte01", "TS_ChB_ERRQ_Byte64", etc.
_BYTE_SIG_RE = re.compile(
...
class ErrqAggregator:
    ...
    The lifecycle (active/passive) is handled by ErrqStateTracker —
    this class only assembles bytes.
    """
    ...
    def ingest(self, signals: dict) -> set[str]:
        """
        Update buffers from a frame's decoded signals. Returns the set of
        channels touched ("A" / "B") so the caller knows whose snapshot
        to grab.
        """
        if not signals:
            return set()
