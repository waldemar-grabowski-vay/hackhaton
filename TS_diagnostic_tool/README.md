...
└── packaging/
    ...
    └── installer.iss  Inno Setup script — produces TSDiag-Setup-*.exe
```
...
3. Runs PyInstaller against `packaging\ts_diag.spec` — output is
   `dist\TSDiag\TSDiag.exe` (a portable folder build, ready to run).
4. Runs `ISCC.exe packaging\installer.iss` — output is
   `dist_installer\TSDiag-Setup-<version>.exe`.
